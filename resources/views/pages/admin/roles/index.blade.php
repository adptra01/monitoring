                            ...
                            <flux:table.row :key="$role->id">
                                ...
                                <flux:table.cell>
                                    <div class="flex gap-2">
                                        <flux:button variant="ghost" size="sm" icon="pencil" href="{{ url('/admin/roles/' . $role->id . '/edit') }}" />
                                        @if ($role->name !== 'admin')
                                            <flux:button variant="ghost" size="sm" icon="trash" wire:click="delete({{ $role->id }})" wire:confirm="{{ __('Are you sure you want to delete this role?') }}" />
                                        @endif
                                    </div>
                                </flux:table.cell>
                            </flux:table.row>
